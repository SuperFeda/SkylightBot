    ...
    async def on_message(self, message):
        ...
        if can_description_var == 1 or can_description_var is True:
            ...
            if len_message_attachments > 10:
                ...
                return await message.channel.send(f"<@{message.author.id}>", view=EnterDescriptionAgainButton(self.client), embed=embed)
            ...
            if len_message_content < 10:
                self.load_can_description_var(user_id=message.author.id)
                embed = utils.create_embed(title="Слишком короткое описание", color=disnake.Color.red(), content="Описание должно быть длинне **10** символов.")
                return await message.channel.send(f"<@{message.author.id}>", view=EnterDescriptionAgainButton(self.client), embed=embed)
